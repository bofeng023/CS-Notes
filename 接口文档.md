...
###### UC应用授权流程为平台部指定app_id、app_secret、redirect_url交给运营同事申请uc的app，由我们调用uc的接口获取access_token。我们再编写一个回调接口来接收access_token参数，将access_token写入日志文件保存，接下来我们就能带着access_token参数来获取uc信息流了<br>
具体流程如下图：<br>
