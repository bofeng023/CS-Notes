### 应用授权文档
<br>
## UC应用授权流程为平台部指定app_id、app_secret、redirect_url交给运营同事申请uc的app，由我们调用uc的接口获取access_token。我们再编写一个回调接口来接收access_token参数，将access_token写入日志文件保存，接下来我们就能带着access_token参数来获取uc信息流了。具体流程如下图：
![](https://doc.open-uc.cn/image/xxl.png)

请求uc的接口为：https://open.uczzd.cn/openiflow/auth/token
请求方式：post
请求参数：
...
<br>
我们对外提供的回调接口为
http://39.106.171.57:8080/uc/token
请求方式：post
请求参数：
